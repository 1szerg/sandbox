...
public class PathfinderTest
{
    public TheMap runMap(int sizeX, int sizeY, int blocks, PathFinderEngine pathFinder)
    {
        ...
        Tile finish = getRandomTile(map, map.sizeY-1);
        start.start = true;
        finish.finish = true;
    ...
    private Tile getRandomTile(TheMap map, int y)
    {
        Random r = new Random();
        for(int limit = map.sizeX * 10; limit > 0; limit--)
        {
            Tile t = map.tile(r.nextInt(map.sizeX), y);
            if(!t.blocked){
                return t;
            }
        }
